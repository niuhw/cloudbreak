...
public interface ProxyConfigV3Endpoint {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    ProxyConfigResponse attachToEnvironments(@NotEmpty Set<String> environmentNames);

    @PUT
    @Path("{name}/detach")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    ProxyConfigResponse detachFromEnvironments(@NotEmpty Set<String> environmentNames);
}
